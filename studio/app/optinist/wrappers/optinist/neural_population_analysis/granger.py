...
class GrangerParams(BaseModel):
    ...
    Granger_addconst: bool = Field(
        True, description="Include a constant in the model or not."
    )
...
class Granger(AlgoTemplate):
    def run(
        ...
        print("start granger:", self.function_id)
        ...
        GC = {
            ...
            "Granger_fval_mat": [
                np.zeros([num_cell, num_cell]) for i in range(num_lag)
            ],
        }
