...
class CrossCorrelationParams(BaseModel):
    ...
    method: str = Field("direct")
    lags: int = Field(100, description="number of frames to show (+-frames)")
    shuffle_sample_number: int = Field(
        100, description="number of cross correlation caliculation for randomization"
    )
    shuffle_confidence_interval: float = Field(0.95)


class CrossCorrelation(AlgoTemplate):
    def run(
        self,
        ...
        print("start cross_correlation:", self.function_id)
        ...
        for i in tqdm(range(num_cell)):
            ...
            for j in tqdm(range(num_cell)):
                ...
                for k in range(shuffle_num):
                    ...
                    ccvals = ss.correlate(
                        X[i, :], tp, method=params["method"], mode="same"
                    )
