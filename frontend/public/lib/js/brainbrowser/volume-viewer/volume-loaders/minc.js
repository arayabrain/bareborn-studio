...
VolumeViewer.createVolume = function (header, native_data) {
  ...
  var volume = {
    ...
    getSliceImage: function (slice, zoom, contrast, brightness) {
      ...
      var source_image = image_creation_context.createImageData(
        slice.width,
        slice.height,
      )
      ...
      var position = {
        xspace:
          header.xspace.step > 0
            ? volume.position.xspace
            : header.xspace.space_length - volume.position.xspace,
        yspace:
          header.yspace.step > 0
            ? volume.position.yspace
            : header.yspace.space_length - volume.position.yspace,
        zspace:
          header.zspace.step > 0
            ? volume.position.zspace
            : header.zspace.space_length - volume.position.zspace,
      }
...
function parseHeader(header_text, callback) {
  ...
  header.xspace.direction_cosines =
    header.xspace.direction_cosines.map(parseFloat)
  header.yspace.direction_cosines =
    header.yspace.direction_cosines.map(parseFloat)
  header.zspace.direction_cosines =
    header.zspace.direction_cosines.map(parseFloat)
  ...
  header[header.order[0]].offset =
    header[header.order[1]].space_length * header[header.order[2]].space_length
