...
BrainBrowser.VolumeViewer.modules.loading = function (viewer) {
  ...
  viewer.loadVolumes = function (options) {
    options = options || {}
    var overlay_options =
      options.overlay && typeof options.overlay === 'object'
        ? options.overlay
        : {}
   ...
   * ```
   */
  viewer.loadVolumeColorMapFromURL = function (
    vol_id,
    url,
    cursor_color,
    callback,
   ...
   * ```
   */
  viewer.loadVolumeColorMapFromFile = function (
    vol_id,
    file_input,
    cursor_color,
    callback,
   ...
   * ```
   */
  viewer.loadDefaultColorMapFromFile = function (
    file_input,
    cursor_color,
    callback,
  ) {
  ...
  function setVolume(vol_id, volume_description, callback) {
    openVolume(volume_description, function (volume) {
      ...
      volume.display = createVolumeDisplay(
        viewer.dom_element,
        vol_id,
        volume_description,
      )
  ...
  function getTemplate(dom_element, vol_id, template_id, viewer_insert_class) {
    var template = document
      ...
      .innerHTML.replace(/\{\{VOLID\}\}/gm, vol_id)
    ...
    ;(function () {
      ...
      views.forEach(function (axis_name) {
        ...
        function mouseUpRotation() {
          panel.volume.header[axis_name].isRotation = false
          if (
