import { Box, Button, styled, Select } from '@mui/material'
...
const ModalComponent = ({data, setData, setIsOpenModal, type, dataEdit }: any) => {
    ...
    const [values, setValues] = useState<{ [key: string]: string }>(dataEdit || {
        email: '',
        password: '',
        role: '',
        lab: '',
    ...
    const onChangeData = useCallback((e : any,type : string) => {
        setFormData({...formData, [type] : e.target.value})
        const { value } = e.target
        setValues({ ...values, [type]: value })
    },[formData])
    const onBlurData = useCallback((e : any,type : string, validate?: Function) => {
        const {value} = e.target
        setValues({ ...values, [type]: value })
        setErrors({ ...errors, [type]: value === "" ? 'This field is required': validate?.(value)})
    },[formData])
    const onSubmit = (e:any) => {
        ...
        const errorNotMatch = formData.password === formData.confirmPassword ? '' : 'password is not match'
        if (errorEmail || errorPassword || errorConfirmPassword || errorNotMatch) {
            setErrors({
                email: errorEmail,
                password: errorPassword,
                confirmPassword : errorConfirmPassword === "" ? errorNotMatch : errorConfirmPassword,
                name: values.name === ''  || !formData.hasOwnProperty('name')? 'This field is required' : '',
                lab: values.lab === '' || !formData.hasOwnProperty('lab')? 'This field is required' : '',
                role: values.role === '' || !formData.hasOwnProperty('role')? 'This field is required' : '',
            })
            return
        }
        if(type === "Add"){
        ...
        <ModalBox>
            ...
            <BoxData>
                ...
                <InputError
                    ...
                    onBlur = {(e:any)=> onBlurData(e,'confirmPassword',validatePassword)}
                    type={'password'}
...
const AccountManager = () => {
  ...
  return (
    <AccountManagerWrapper>
      ...
      />
      <BoxButton>
        <ButtonAdd onClick = {()=>onOpenModal('Add')} variant="contained">Add</ButtonAdd>
      </BoxButton>
      <TableComponent
...
const SelectModal = styled(Select)(({theme})=>({
    width : 272,
    marginBottom : '15px',
