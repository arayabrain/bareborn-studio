...
const ProjectFormComponent = () => {
  ...
  const onFilter = (value: { [key: string]: string }) => {
    ...
    const newParams = Object.keys(value)
      ...
      .join('&')
    idEdit ? setParams(`id=${idEdit}&${newParams}${routeGoback ? `&back=${routeGoback}` : ''}`) : setParams(newParams)
  }
  ...
  const handleClear = () => {
    idEdit ? setParams(`id=${idEdit}${routeGoback ? `&back=${routeGoback}` : ''}`) : setParams('')
    const data = onSort(
