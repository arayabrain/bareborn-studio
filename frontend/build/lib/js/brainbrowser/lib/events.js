  ...
  addEventModel: function (object) {
    ...
    object.propagateEventTo = function (event_name, other) {
      if (!BrainBrowser.utils.isFunction(other.allPropagationTargets)) {
        throw new Error(
          "Propagation target doesn't seem to have an event model.",
        )
      }

      if (
        object === BrainBrowser.events ||
        other.allPropagationTargets(event_name).indexOf(object) !== -1
      ) {
        throw new Error(
          "Propagating event '" + event_name + "' would cause a cycle.",
        )
      }
    ...
    object.stopPropagatingTo = function (other) {
      Object.keys(propagated_events).forEach(function (event_name) {
        propagated_events[event_name] = propagated_events[event_name].filter(
          function (target) {
            return target !== other
          },
        )
    ...
    object.allPropagationTargets = function (event_name) {
      ...
      direct_targets.forEach(function (target) {
        target
          .allPropagationTargets(event_name)
          .forEach(function (recursive_target) {
