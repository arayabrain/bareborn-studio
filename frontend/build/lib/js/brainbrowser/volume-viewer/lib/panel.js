...
;(function () {
  ...
  window.BrainBrowser.VolumeViewer.createPanel = function (options) {
        ...
        return {
          x:
            volume.position[slice.width_space.name] *
              Math.abs(slice.width_space.step) *
              panel.zoom +
            origin.x,
          y:
    ...
    if (panel.volume) {
      ...
      panel.default_zoom = volume.getPreferredZoom(
        panel.canvas.width,
        panel.canvas.height,
      )
      panel.zoom = panel.default_zoom
  ...
  function setSlice(panel, slice) {
    panel.slice = slice
    panel.slice_image = panel.volume.getSliceImage(
      panel.slice,
      panel.zoom,
      panel.contrast,
      panel.brightness,
  ...
  function getDrawingOrigin(panel) {
    ...
    return {
      x:
        panel.image_center.x -
        Math.abs(
          slice.width_space.step * slice.width_space.space_length * panel.zoom,
        ) /
          2,
      y:
        panel.image_center.y -
        Math.abs(
